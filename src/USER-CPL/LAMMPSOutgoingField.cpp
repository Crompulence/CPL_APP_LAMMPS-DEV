...
void TemperatureOutgoingField::pack_(const std::vector<int>& glob_cell,
    ...
    if (bc_dimension == "3d" || bc_dimension == "3D") {
        ...
        T = 1.0;// cfdbcfix->compute_array(row, 7);  
    }
    // For 1D case
    else {
       T = 1.0;//cfdbcfix->compute_array(0, 7);  
    }
            ...
            if (CPL::is_proc_inside(bc_region.data())) {
                ...
                for (int k = bc_region[4]; k <= bc_region[5]; k++) {
                    row = i*region.nCells[1]\
                           *region.nCells[2] + j\
                           *region.nCells[2] + k; 
                    vAvg[0] += 0.0;//cfdbcfix->compute_array(row, 4);
                    vAvg[1] += 0.0;//cfdbcfix->compute_array(row, 5);
                    vAvg[2] += 0.0;//cfdbcfix->compute_array(row, 6);
                }
                vAvg = vAvg / double(N);
...
void VelOutgoingField::pack_(const std::vector<int>& glob_cell,
    ...
    if (bc_dimension == "3d" || bc_dimension == "3D") {
        ...
        if (averageVels) {
            ...
            vx = 0.0;//cfdbcfix->compute_array(row, 4);  
            vy = 0.0;//cfdbcfix->compute_array(row, 5);  
            vz = 0.0;//cfdbcfix->compute_array(row, 6);  
        }
    }
    // For 1D case
    else {
        vx = 0.0;//cfdbcfix->compute_array(0, 4);  
        vy = 0.0;//cfdbcfix->compute_array(0, 5);  
        vz = 0.0;//cfdbcfix->compute_array(0, 6);  
    }
...
void NbinOutgoingField::pack_(const std::vector<int>& glob_cell,
    ...
    if (bc_dimension == "3d" || bc_dimension == "3D") {
        int row = glob_cell[0]*region.nCells[1]\
                              *region.nCells[2] + glob_cell[1]\
                              *region.nCells[2] + glob_cell[2];
        ncount = 1; //cfdbcfix->compute_array(row, 3);  
    }
    else {
	    ncount = 1; //cfdbcfix->compute_array(0, 3);  
    }
